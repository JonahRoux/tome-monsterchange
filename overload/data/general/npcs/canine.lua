...
newEntity{ base = "BASE_NPC_CANINE",
	...
	is_pack_member = true,
	pack_member_rarity = 1,
	level_range = {1, nil}, exp_worth = 0,
	max_life = resolvers.rngavg(40,70),
	combat_armor = 1, combat_def = 3,
	combat = { dam=resolvers.levelup(15, 1, 1.2), atk=0, apr=10 },
	resolvers.talents{
...
newEntity{ base = "BASE_NPC_CANINE",
	name = "greater wolf", color=colors.UMBER, image="npc/canine_gw.png",
	desc = [[Larger than a normal wolf, it prowls and snaps at you. It seems to be leading the pack.]],
	...
	--Create Pack members and add them to the pack table
	on_added_to_level = function(self)
		local pack = {}
		for i = 1, 3 do
			local x, y = util.findFreeGrid(self.x, self.y, 15, true, {[engine.Map.ACTOR]=true})
			if x and y then
				local m = game.zone:makeEntity(game.level, "actor", {properties = {"is_pack_member"}, special_rarity = "pack_member_rarity"}, nil, true)
				if m then
					m.summoner = self
	...
	--Weaken the pack on the death of its leader
	on_die = function(self, src)
		local nb = 0
		for member, _ in pairs(self.pack) do
			if not member.dead then member:addTemporaryValue("generic_damage_penalty", 30) nb = nb + 1 end
		end
		if nb > 0 then
			self:logCombat(self, "#AQUAMARINE#As %s falls its pack weakens!", self.name)
		end
	end,
}
...
newEntity{ base = "BASE_NPC_CANINE",
	...
	combat = { dam=resolvers.levelup(20, 1, 1.5), atk=5, apr=15 },
	resolvers.talents{
		[Talents.T_RUSH]={base=0, every=10},
		[Talents.T_KNOCKBACK]={base=0, every=8},
		[Talents.T_HOWL]=1,
...
newEntity{ base = "BASE_NPC_CANINE",
	...
	make_escort = {
		{type="animal", subtype="canine", name="warg", number=2, no_subescort = true},
	},
